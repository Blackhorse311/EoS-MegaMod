...
--[[------------------------------------------------------------------------------
    ...
    Data from behaviour accessed via 'them' (NPC actor).

    them.lawyerOnCooldown   - boolean, 7-day cooldown on heat reduction
    them.judgeBribed         - boolean, one-time judge bribe flag
    them.pendingHeatReduction - boolean, set by conversation, read by behaviour
    them.pendingJudgeBribe    - boolean, set by conversation, read by behaviour
--------------------------------------------------------------------------------]]
...
--[[------------------------------------------------------------------------------
    ...
    - Processes pending heat reduction (applies police activity effect)
    - Processes judge bribe (applies permanent small reduction)
    - Manages 7-day cooldown on heat reduction
...
function onAdd()
    ...
    thisActor.lawyerOnCooldown = false
    thisActor.judgeBribed = false
    thisActor.pendingHeatReduction = false
    thisActor.pendingJudgeBribe = false
    thisActor.lawyerCooldownTimestamp = nil
    thisActor.lawyerPendingCooldown = false
end

-- Helper: apply police activity effect to all player neighborhoods
local function applyPoliceEffect(amount)
    local playerFaction = WorldUtils:getPlayerFaction()
    ...
    for i = 1, #buildings do
        ...
        if building and building.neighborhood then
            local hoodId = building.neighborhood.id or building.neighborhood
            if not processed[hoodId] then
                processed[hoodId] = true
...
function GameEvent.frame(e)
    ...
    if thisActor.pendingHeatReduction then
        thisActor.pendingHeatReduction = false
        thisActor.lawyerPendingCooldown = true
        applyPoliceEffect(HEAT_REDUCTION_AMOUNT)
    end

    -- Process judge bribe (one-time permanent effect)
...
function GameEvent.onDayBegin(e)
    -- Record cooldown timestamp (client.time.worldTime confirmed available here)
    if thisActor.lawyerPendingCooldown then
        thisActor.lawyerPendingCooldown = false
        thisActor.lawyerCooldownTimestamp = client.time.worldTime
    end

    -- Check if heat reduction cooldown has expired
    if thisActor.lawyerCooldownTimestamp then
        local elapsed = client.time.worldTime - thisActor.lawyerCooldownTimestamp
