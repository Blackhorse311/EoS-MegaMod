...
function Mission:load()
    ...
    if self._gangstersInvolvedInMissions and #self._gangstersInvolvedInMissions > 0 then
        game:addEventListener("onRemoveRPC", self)
...
local function removeGangsterFromMissionImpl(self, gangster)
    ...
    if self._gangstersInvolvedInMissions and #self._gangstersInvolvedInMissions == 0 then
        game:removeEventListener("onRemoveRPC", self)
...
function Mission:onRemoveRPC(e)
    local removedRPC = e.rpc
...
function Mission:complete(succeeded, reasonText, reasonParams)
    ...
    -- Don't raise the onMissionComplete event for combat missions.
    if not self._isCombatMission then
...
function Missions:createMission(missionId, ...)
    ...
    if mission then
        ...
        if not self._focusedMission then
...
function Missions:focusNewMission()
    if #self._activeMissions > 0 then
        self:setFocusedMission(self._activeMissions[1])
    else
        self:setFocusedMission(nil)
    end
end
...
function Missions:setFocusedMission(mission)
    ...
    if previousFocused then previousFocused:unfocus() end
